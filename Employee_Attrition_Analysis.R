# ************************************************
# R Script For Employee Attrition Analysis Coursework

#  clears the objects in global environment
rm(list=ls())
...
implement_neural_network <- function(dataset, numeric_cols) {
  ...
  if (length(missing_cols) > 0) {
    cat("These columns are missing from data frame:", paste(missing_cols, collapse = ", "), "\n")
  }
  ...
  borderPrint("Training Neural Network Model")
...
implement_neural_network_cv <- function(dataset, numeric_cols){
  ...
  if (length(missing_cols) > 0) {
    cat("These columns are missing in the data frame:", paste(missing_cols, collapse = ", "), "\n")
  }
...
# ************************************************
# implement_decision_tree (training_data, testing_data, min_bucket, min_split, max_depth) :
#
...
# ************************************************
implement_decision_tree <- function(training_data, testing_data, min_bucket, min_split, max_depth) {
  
  dt_model= rpart(Attrition~., data = training_data, method="class",
                  minbucket = min_bucket, minsplit = min_split, maxdepth = max_depth)
...
# ************************************************
# implement_random_forest (training_data, testing_data, ntree, nodesize) :
#
# Implements the Random Forest model
#
# INPUT: training_data - dataset used for training the model
#        testing_data - dataset used for testing the model
#        ntree - no. of trees for this model
#        nodesize - no. of nodes for this model
#
# OUTPUT : Classification Report of Random Forest model
# ************************************************
implement_random_forest <- function(training_data, testing_data, trees, node_size) {
  ...
  randomForestModel <- randomForest(Attrition ~ ., data = training_data_RF, ntree = trees, nodesize = node_size, mtry=3)
...
# main() :
#
# This is where the main execution of the project happens
# ************************************************
main<-function(){
  ...
  # Creating 70/30 partition 
  train_index <- createDataPartition(y = CombinedDateForML$Attrition,  
                                     p = .7,
                                     list = FALSE, 
                                     times = 1)  # Sets number of partitions to create to 1. 
  training_data <- CombinedDateForML[train_index,]
  testing_data <- CombinedDateForML[-train_index,]
  
  # Creating 80/20 partition 
  train_index_80 <- createDataPartition(y = CombinedDateForML$Attrition,  
                                     p = .8,
                                     list = FALSE, 
                                     times = 1)  # Sets number of partitions to create to 1. 
  training_data_80 <- CombinedDateForML[train_index_80,]
  testing_data_80 <- CombinedDateForML[-train_index_80,]
  ...
  #                           Decision Tree Model
  # *********************************************************************
  # Abdullah's implementation of this model
  DT_classification_report_Abdullah <- implement_decision_tree(training_data,
                                                               testing_data,
                                                               min_bucket = 20,
                                                               min_split = 10,
                                                               max_depth = 15) 
  
  # Fereniki's implementation of this model
  DT_classification_report_Fereniki <- implement_decision_tree(training_data_80,
                                                               testing_data_80,
                                                               min_bucket = 25,
                                                               min_split = 15,
                                                               max_depth = 20)
  ...
  #                           Random Forest Model
  # *********************************************************************
  # Hasan's implementation of this model
  RF_classification_report_Hasan <- implement_random_forest(training_data_80,
                                                            testing_data_80,
                                                            trees = 58,
                                                            node_size = 10)
  
  # Fazail's implementation of this model
  RF_classification_report_Fazail <- implement_random_forest(training_data,
                                                             testing_data,
                                                             trees = 80,
                                                             node_size = 12)
  ...
  NN_performance <- data.frame(Description = c("Before CV", "After CV"),
                               performance_score = unlist(performance_score),
                               performance_score_with_cv = unlist(performance_score_with_cv))
  pander(NN_performance)
  ...
  classification_reports_combined <- c(list(LR_classification_report_testing),
                                       list(DT_classification_report_Abdullah),
                                       list(DT_classification_report_Fereniki),
                                       list(RF_classification_report_Hasan),
                                       list(RF_classification_report_Fazail),
                                       list(SVM_classification_report_testing),
                                       list(KNN_classification_report_testing),
                                       list(NB_classification_report_testing),
                                       list(NN_classification_report_with_cv))
  ...
  model_names <- c("Logistic Regression",
                   "Decision Tree by Abdullah",
                   "Decision Tree by Fereniki",
                   "Random Forest by Hasan",
                   "Random Forest by Fazail",
